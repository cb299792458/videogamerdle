            ...
            <div id='board'>
                {grid.map( (line,r) => {
                    return <div id='row' key={r}>
                        {line.map( (tile,c) => {
                            return <div className='tile' key={c} id={[r,c]} style={colors[r][c][0] ? {backgroundColor: colors[r][c][0], color: 'white'} : {}} onMouseDown={selectOrigin} onMouseUp={selectDestination}>
                                {grid[r][c]}
                            </div>
                        })}
                    </div>
